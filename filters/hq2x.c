...
#define DECOMPOSE_PIXEL(PIX, R, G, B)       { (B) = (PIX) >> 11; (G) = ((PIX) >> 6) & 0x1f; (R) = (PIX) & 0x1f; }
...
static inline uint8_t Diff (int, int);


void menu_filter_hq2x_init (void)
{
	RGBtoYUV = calloc(1 << ((FIRST_COLOR_MASK & 0x8000) ? 16 : 15), sizeof(int));
	if (!RGBtoYUV)
		return;
...
static void InitLUTs (void)
{
	...
	uint32_t	n = 1 << ((FIRST_COLOR_MASK & 0x8000) ? 16 : 15);
	for (c = 0 ; c < n ; c++)
	{
...
static inline uint8_t Diff (int c1, int c2)
{
...
void filter_hq2x (const uint16_t *p_srcPtr, uint16_t *p_dstPtr, unsigned int width, unsigned int srcPitch, unsigned int dstPitch, unsigned int height)
{
	uint8_t *srcPtr = (uint8_t *)p_srcPtr;
	uint8_t *dstPtr = (uint8_t *)p_dstPtr;
	register int	w1, w2, w3, w4, w5, w6, w7, w8, w9;
	register uint32_t	src1line = srcPitch >> 1;
	register uint32_t	dst1line = dstPitch >> 1;
	register uint16_t	*sp = (uint16_t *) srcPtr;
	register uint16_t	*dp = (uint16_t *) dstPtr;
...
void filter_hq3x (const uint16_t *p_srcPtr, uint16_t *p_dstPtr, unsigned int width, unsigned int srcPitch, unsigned int dstPitch, unsigned int height)
{
	uint8_t *srcPtr = (uint8_t *)p_srcPtr;
	uint8_t *dstPtr = (uint8_t *)p_dstPtr;
	register int	w1, w2, w3, w4, w5, w6, w7, w8, w9;
...
void filter_hq4x (const uint16_t *p_srcPtr, uint16_t *p_dstPtr, unsigned int width, unsigned int srcPitch, unsigned int dstPitch, unsigned int height)
{
	uint8_t *srcPtr = (uint8_t *)p_srcPtr;
	uint8_t *dstPtr = (uint8_t *)p_dstPtr;
	register int	w1, w2, w3, w4, w5, w6, w7, w8, w9;
